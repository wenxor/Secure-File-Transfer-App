...
public class ServerMain {
    ...
    private static void setMaxSizeUpload(String cmd) {
        ...
        ControlThread.UPLOAD_MAX_SIZE = (long) (Long.parseLong(cmd.substring(cmd.indexOf(" ") + 1).trim())
                * Math.pow(1024, 2));
    ...
    private static void showAllUser() {
        ...
        String format = "| %-" + maxIdLength + "s | %-" + maxFullnameLength + "s | %-" + maxUsernameLength + "s | %-"
                + maxEmailLength + "s | %-" + maxMaxSizeLength + "s | %-" + maxDateCreatedLength + "s | %-"
                + maxAnonymousLength + "s | %-" + maxActivatedLength + "s |%n";

        System.out.printf(format, "ID", "Full name", "Username", "Email", "Capacity", "Date created",
                "Anonymous allowed", "Email activated");

        for (int i = 0; i < maxIdLength + maxFullnameLength + maxUsernameLength + maxEmailLength + maxMaxSizeLength
                + maxDateCreatedLength + maxAnonymousLength + maxActivatedLength + 26; i++) {
            System.out.print("-");
        }
        System.out.println();

        for (User user : users) {
            System.out.printf(format, user.getId(), user.getFullname(), user.getUsername(), user.getEmail(),
                    Math.round(user.getMax_size() / Math.pow(1024, 2)) + "MB", user.getDate_created(),
                    user.isAnonymous() ? "ALLOWED" : "BLOCKED", user.isActivated() ? "ACTIVATED" : "NON-ACTIVE");
        }
    }
